...
def getAllTravelMapData():
    travelMapData = db('select * from travelinfo', [], 'select')
    return travelMapData

def getUser_ratings():
    user_ratings = {}
    travelist=getAllTravelMapData()
    for travel in travelist:
        comments = json.loads(travel[15])
        for com in comments:
            try:
                com['userId']
            except:
            ...
            if user_ratings.get(com['userId'],-1) == -1:
                user_ratings[com['userId']] = {travel[1]:com['score']}
            else:
                user_ratings[com['userId']][travel[1]] = com['score']
    return user_ratings
